...
class FlxTypedButton<T:FlxSprite> extends FlxSprite implements IFlxInput {
  ...
  private function checkInput(pointer:FlxPointer, input:IFlxInput, justPressedPosition:FlxPoint, camera:FlxCamera):Bool {
    if(maxInputMovement != Math.POSITIVE_INFINITY && justPressedPosition.distanceTo(pointer.getScreenPosition(FlxPoint.weak())) > maxInputMovement && input == currentInput) {
      currentInput == null;
    }
...
/** 
 * イベントをハンドルするFlxButtonのためのヘルパー関数
 */
private class FlxButtonEvent implements IFlxDestroyable {
  /**
   * このイベントを実行するときに呼び出されるコールバック関数
   **/
  ...
  /**
   * @parma Callback このイベントで実行するコールバック関数
   * @param sound    このイベントを実行したときのサウンド
   **/
  ...
  /**
   * このイベントを実行する (コールバックの呼び出し。サウンドの再生)
   **/
